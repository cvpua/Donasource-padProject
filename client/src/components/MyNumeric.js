...
const MyNumeric = (props) => {
	...
	return (
		<Field name={`${name}[${index}].donor[donor.length - 1].amountDonated`}>
			{
				({field,form}) => {
					...
					<FormControl
						isInvalid={
							...
							((
								...
								form.errors[name][index].donor[donorLength].amountDonated && 
								form.touched[name][index].donor[donorLength].amountDonated
							) ||
							(
						...
						mb="4"
					>
						<FormLabel htmlFor={`${name}[${index}].donor[${donorLength}].amountDonated`}>{label}</FormLabel>
						<NumberInput 
							...
							onChange={ (val) => {
								...
								form.setFieldValue(`${name}[${index}].donor[${donorLength}].amountDonated`, val)
							}} 
							onBlur={ (e) => {form.setFieldTouched(`${name}[${index}].donor[${donorLength}].amountDonated`,true)}} />
						<FormErrorMessage>
							{
								form.errors[name] && 
								form.errors[name][index] &&
